...
- 不同狀態顯示不同樣式時，程式碼需要封裝
  ...
  - 當元件的樣式比較複雜時，例如按鈕在 Default/Primary/Danger 會有不同樣式，同時不同尺寸，像是 Default/Small/Large 也會有不同的樣式，那要怎麼寫比較好？當然功能上是都可以達到，但是想要思考看看有沒有更好的寫法？
- RWD 元件開發複雜
...
不要對熟悉的工具有過度的依賴，當然也不要為了追求新技術而去使用，大家可以思考 Styled System 是不是真的適合應用在目前的專案裡？ 或許 Styled System 不是你現在的靈藥，但是可以去希望是你其中一個很棒的選擇 ：）
